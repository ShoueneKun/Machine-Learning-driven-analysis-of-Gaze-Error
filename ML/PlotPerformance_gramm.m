...
% [11, 14, 24, 34, 44];
Clx_Names = {'RF'; 'biRNN'; 'RNN_onlyEyes'; 'RNN_onlyAbs'; 'fRNN'};

load('PerformanceMatrix.mat')
load('PerformanceMatrix_kfold.mat')
load('kappaResults_kfold.mat')
...
Clx_plot = [1, 1, 0, 0, 1, 0];
list_WinSizes = 0:3:24;

Num_WinSizes = 9;
A = cell2mat(HumanSample_Perf(:, 1)); B = cell2mat(HumanSample_Perf(:, 6));
WinSizes = repmat(list_WinSizes, [length(A), 1]); WinSizes = {WinSizes(:)}; 
...
g(1,1) = gramm('x', (2*WinSizes+1)/SR, 'y', A, 'color', colors);
g(1,2) = gramm('x', (2*WinSizes+1)/SR, 'y', B(:, 1), 'color', colors);
g(1,3) = gramm('x', (2*WinSizes+1)/SR, 'y', B(:, 2), 'color', colors);
g(1,4) = gramm('x', (2*WinSizes+1)/SR, 'y', B(:, 3), 'color', colors);
