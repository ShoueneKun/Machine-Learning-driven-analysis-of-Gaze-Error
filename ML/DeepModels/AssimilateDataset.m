    ...
    for j = 1:length(TrPresent)
        ...
        if sum(loc2) > 1 && sum(LbrPresent) > 1
            ...
            if CombLabels
                ...
                LabelMat = cell2mat(Data_PrTrLbr.Labels');
                sc_freq = LabelMat ~= 0 & LabelMat ~=4; % All locations where a valid label is present
                sc_freq = sum(sc_freq, 2);
                weight_scale = 1./sc_freq; weight_scale(isinf(weight_scale)) = 1;
                
                for g = 1:size(Data_PrTrLbr, 1) % Loop through available labellers
                    datamat = cell2mat(table2array(Data_PrTrLbr(g, :)));
                    Conf = cell2mat(Data_PrTrLbr(g, :).Conf); % Per sample confidence.
                    
                    % To be fair to the algorithm, repeats due to labelling
                    % is an artifact induced by the labelling strategy.
